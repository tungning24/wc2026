...
function renderBracket(rounds){
  ...
  if(window.matchMedia("(max-width: 700px)").matches){
    renderMobileBracketTabs(rounds);
    return;
  }
...
function renderMobileBracketTabs(rounds){
  const nextMatch = {
    74:89,77:89,73:90,75:90,
    76:91,78:91,79:92,80:92,
    83:93,84:93,81:94,82:94,
  ...
  const availableTabs = roundTabs.filter(([roundName])=>{
    return grouped[roundName] && grouped[roundName].length;
  });

  const wrap = document.createElement("div");
  wrap.className = "mobile-bracket-tabs-wrap";
  wrap.innerHTML = `
    <div class="mobile-bracket-tabs" role="tablist">
      ${availableTabs.map(([roundName, label], index)=>`
        <button
          class="mobile-bracket-tab${index === 0 ? " active" : ""}"
          type="button"
          data-round="${escapeAttr(roundName)}"
          role="tab"
        >${label}</button>
      `).join("")}
    </div>
    <div class="mobile-bracket-panel"></div>
  `;

  container.appendChild(wrap);

  const panel = wrap.querySelector(".mobile-bracket-panel");
  const buttons = Array.from(wrap.querySelectorAll(".mobile-bracket-tab"));
  ...
  function renderRound(roundName){
    ...
    let html = `<div class="mobile-round-title">${roundName}</div>`;

    matches.forEach(match=>{
      const homeFlag = flagMap[match.home] || "none";
      const awayFlag = flagMap[match.away] || "none";
      const winnerText = nextMatch[match.num]
        ? `<div class="mobile-advance">Winner to M${nextMatch[match.num]}</div>`
        : "";

      html += `
        <div class="mobile-ko-match">
          <div class="ko-match-header">
            <span>M${match.num} ${match.date}</span>
            <span>${match.time}</span>
          </div>
